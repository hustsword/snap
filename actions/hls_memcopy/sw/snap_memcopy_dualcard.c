...
static struct snap_action* prepare_card (int card_no, snap_action_flag_t action_irq, struct snap_card* card)
{
...
int main (int argc, char* argv[])
{
    ...
    int card_no = 0;
    int action_no = 0;
    struct snap_action* action_0 = NULL;
    struct snap_action* action_1 = NULL;
    struct snap_card* card_0 = NULL;
    struct snap_card* card_1 = NULL;
    //char device[128];
    struct snap_job cjob_0;
    struct memcopy_job mjob_0;
    struct snap_job cjob_1;
    struct memcopy_job mjob_1;
    ...
    tbuff = snap_malloc (set_size);

    if (tbuff == NULL) {
        goto out_error;
    }

    memset (tbuff, 0x0, set_size);
    addr_temp = (unsigned long)tbuff;
    ...
    action_no = 0;
    action_0 = prepare_card (action_no, action_irq, card_0);
    if (action_0 == NULL) {
        fprintf (stderr, "err: failed to attach action %u: %s\n",
                 card_no, strerror (errno));
        goto out_error2;
    ...
    action_no = 1;
    action_1 = prepare_card (action_no, action_irq, card_1);
    if (action_1 == NULL) {
        fprintf (stderr, "err: failed to attach action %u: %s\n",
                 card_no, strerror (errno));
        goto out_error2;
    ...
    // Card 0, copy from addr_in to addr_temp
    snap_prepare_memcopy (&cjob_0, &mjob_0,
                          (void*)addr_in,  size, type_in,
                          (void*)addr_temp, size, type_out);

    __hexdump (stderr, &mjob_0, sizeof (mjob_0));
    ...
    rc = snap_action_sync_execute_job (action_0, &cjob_0, timeout);
    ...
    /* obuff[size] = 0xff; */
    (cjob_0.retc == SNAP_RETC_SUCCESS) ? fprintf (stdout, "Card 0 SUCCESS\n") : fprintf (stdout, "Card 0 FAILED\n");

    if (cjob_0.retc != SNAP_RETC_SUCCESS) {
        fprintf (stderr, "err: Unexpected RETC=%x!\n", cjob_0.retc);
        goto out_error2;
    }

    // Card 1, copy from addr_in to addr_temp
    snap_prepare_memcopy (&cjob_1, &mjob_1,
                          (void*)addr_temp,  size, type_in,
                          (void*)addr_out, size, type_out);

    __hexdump (stderr, &mjob_1, sizeof (mjob_1));
    ...
    // the specified action.
    // => timing will thus take into account the registers transfer time added to the action duration
    rc = snap_action_sync_execute_job (action_1, &cjob_1, timeout);
    gettimeofday (&etime, NULL);
    printf ("      got end of exec. time\n");
    ...
    /* obuff[size] = 0xff; */
    (cjob_1.retc == SNAP_RETC_SUCCESS) ? fprintf (stdout, "Card 1 SUCCESS\n") : fprintf (stdout, "Card 1 FAILED\n");

    if (cjob_1.retc != SNAP_RETC_SUCCESS) {
        fprintf (stderr, "err: Unexpected RETC=%x!\n", cjob_1.retc);
        goto out_error2;
    }
    ...
    if (temp != NULL) {
        fprintf (stdout, "writing temp data %p %d bytes to %s\n",
                 tbuff, (int)size, temp);

        rc = __file_write (temp, tbuff, size);
    ...
    snap_detach_action (action_0);
    snap_detach_action (action_1);
    snap_card_free (card_0);
    snap_card_free (card_1);
