...
BufRegex::BufRegex (int in_id, int in_timeout)
...
void* BufRegex::get_src_ptr()
{
    return m_src;
}

void* BufRegex::get_dest_ptr()
{
    return m_dest;
}

size_t BufRegex::get_src_size()
{
    return m_src_size;
}

size_t BufRegex::get_dest_size()
{
    return m_dest_size;
}

void* BufRegex::aalloc (int align, int size)
{
    void* a;
...
int BufRegex::allocate (size_t in_src_size, size_t in_dest_size)
{
...
void BufRegex::work_with_job (JobPtr in_job)
{
    JobRegexPtr job = boost::dynamic_pointer_cast<JobRegex> (in_job);

    if (job == NULL) {
        std::cerr << "Failed to get pointer to JobRegex" << std::endl;
        return;
    }

    //if (m_src == NULL || m_dest == NULL) {
    //    std::cerr << "NULL src or dest pointer in BufRegex" << std::endl;
    //    return;
    //}
    ...
    do {
        if (0 != job->run()) {
            std::cerr << "Failed to run the JobRegex" << std::endl;
            return;
        }
    } while (0);
...
int BufRegex::check()
{
    ...
    for (size_t i = 0; i < m_jobs.size(); i++) {
        JobRegexPtr job = boost::dynamic_pointer_cast<JobRegex> (m_jobs[i]);
