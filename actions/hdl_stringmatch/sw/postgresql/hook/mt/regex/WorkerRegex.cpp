...
WorkerRegex::WorkerRegex (HardwareManagerPtr in_hw_mgr, bool in_debug)
...
void WorkerRegex::set_job_start_threshold (int in_threshold)
{
    m_job_start_threshold = in_threshold;
}

int WorkerRegex::check()
{
    int total_num = 0;

    for (int i = 0; i < (int)m_bufs.size(); i++) {
        BufRegexPtr buf = boost::dynamic_pointer_cast<BufRegex> (m_bufs[i]);
...
int WorkerRegex::setup_completion_queue (int in_size)
{
    ...
    std::cout << "Completion queue size " << std::dec << m_completion_queue_size << std::endl;

    return 0;
}

int WorkerRegex::check_completion_queue ()
{
    if (NULL == m_completion_queue_ptr) {
...
void WorkerRegex::append_job_desc (void* in_job_desc)
{
...
void WorkerRegex::dump_job_desc()
{
    int count = 0;
    JobDescriptor* job_desc_ptr = (JobDescriptor*) m_job_desc_head;
...
void WorkerRegex::check_buf_done()
{
    if (NULL == m_completion_queue_ptr) {
        std::cerr << "Invalid pointer to completion queue" << std::endl;
